!!/////////////////////////////////////////////
!! This program is for solving Fokker-Planck equation
...
integer*4 max_t
parameter (max_t=100000) ! temporal discretization (maX)
	
integer max_s ! spatial discretization (max)
parameter (max_s=500) ! time discretization (maX)
...
integer*4  Nt ! number of timesteps -- actual discretization by time
integer*4  N  ! actual number of  -- actual spatial discretization 
...
! in ccase can be read from command line 

open(1,file = 'new_input.txt') 
read(1,*)N,z 
! N - actual number of segments 
! z -- actual length of the field (related to dt and the mobility constant)

read(1,*)Nt,dt,timedisplay,intdisplay
! Nt -- actual number of timesteps
! dt timestep (conversion into real units discussed) 
! timedisplay -- output withing certain number of steps
! intdisplay -- interval (output once in a certain number of steps)  

do i=0,N
 read(1,*)zN(i),Ft(i)
end do
! zN coordinate for each point, just put zN(i)=i
! Ft is the free energy profile
...
do j=0,timedisplay,intdisplay
    write(2,'(f12.4,2e13.6)')j*dt,abs(pTimeN(j)),abs(pTime0(j)) !,wmt(1,j),wmt(N-1,j) !,wmt(j,2),wmt(j,1000)
enddo
close(2)
